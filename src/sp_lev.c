...
static void undig_corridor(int, int, schar, schar);
static void fix_stair_rooms(void);
static void create_corridor(corridor *);

static boolean create_subroom(struct mkroom *, xchar, xchar, xchar, xchar, xchar, xchar);
...
static void
get_location(schar *x, schar *y, int humidity)
{
	int cpt = 0;
...
static void
get_free_room_loc(schar *x, schar *y, struct mkroom *croom)
{
	schar try_x, try_y;
...
load_rooms(dlb *fd)
{
	...
	for (i=0;i<nrooms;i++) {
		...
		/* read the forges */
		Fread((void *) &r->nforge, 1,
			sizeof(r->nforge), fd);
		if ((n = r->nforge) != 0)
		    r->forges = NewTab(forge, n);
		while (n--) {
			r->forges[(int)n] = New(forge);
			Fread((void *) r->forges[(int)n], 1,
				sizeof(forge), fd);
		}
...
load_maze(dlb *fd)
{
	...
	Fread((void *) &n, 1, sizeof(n), fd);
						/* Number of forges */
	while (n--) {
		Fread((void *)&tmpforge, 1, sizeof(tmpforge), fd);
