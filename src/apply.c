...
static int use_breaking_wheel(struct obj *);
static struct obj *pg_floor_corpse(void);
static boolean pg_spellbook_feed(struct obj *, struct obj *, boolean);
static boolean pg_bullwhip_snag(struct obj *, int, int);
static int use_pest_glaive(struct obj *);
...
use_breaking_wheel(struct obj *obj)
{
	...
	start_timer(20L, TIMER_OBJECT, SLOW_WHEEL, (void *)obj);
	update_inventory();
	return MOVE_PARTIAL;
}
...
static boolean
carrying_silverknight_runeable(void)
{
	struct obj *obj;
	for(obj = invent; obj; obj = obj->nobj){
...
static boolean
carrying_silverknight_weapon(void)
{
	struct obj *obj;
	for(obj = invent; obj; obj = obj->nobj){
